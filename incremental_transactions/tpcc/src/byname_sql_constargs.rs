...
> {
    let (circuit, streams) = Runtime::init_circuit(cconf, |circuit| {
        ...
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={},CUST_ARRAY=ARRAY_AGG($0) WITHIN GROUP ([1]))
        // DBSPStreamAggregateOperator 5043(541)
        let stream5043: Stream<_, IndexedWSet<Tup0, Tup1<Vec<Option<i32>>>>> =
        ...
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={},CUST_ARRAY=ARRAY_AGG($0) WITHIN GROUP ([1]))
        // DBSPMapOperator 5045(558)
        let stream5045: Stream<_, WSet<Tup1<Vec<Option<i32>>>>> = stream5043.map(
            move |t_6: (&Tup0, &Tup1<Vec<Option<i32>>>)| -> Tup1<Vec<Option<i32>>> {
                Tup1::new((*t_6.1).0.clone())
            },
        );
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={},CUST_ARRAY=ARRAY_AGG($0) WITHIN GROUP ([1]))
        // DBSPMapOperator 5050(571)
        let stream5050: Stream<_, WSet<Tup1<Vec<Option<i32>>>>> = stream5043.map(
            move |t_6: (&Tup0, &Tup1<Vec<Option<i32>>>)| -> Tup1<Vec<Option<i32>>> {
                Tup1::new(vec![])
            },
        );
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={},CUST_ARRAY=ARRAY_AGG($0) WITHIN GROUP ([1]))
        // DBSPNegateOperator 5055(574)
        let stream5055: Stream<_, WSet<Tup1<Vec<Option<i32>>>>> = stream5050.neg();
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={},CUST_ARRAY=ARRAY_AGG($0) WITHIN GROUP ([1]))
        ...
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={},CUST_ARRAY=ARRAY_AGG($0) WITHIN GROUP ([1]))
        // DBSPSumOperator 5057(584)
        let stream5057: Stream<_, WSet<Tup1<Vec<Option<i32>>>>> =
            stream580.sum([&stream5055, &stream5045]);
        ...
        // DBSPSinkOperator 5059(598)
        let handle5059 = stream5057.output();
        ...
        // DBSPMapIndexOperator 875
        let stream875: Stream<
            _,
            IndexedWSet<
                Tup0,
                Tup21<
        ...
        // rel#132:LogicalJoin.(left=LogicalTableScan#67,right=LogicalTableScan#69,condition==(+(+($0, $2), $1), ITEM($21, +(/(ARRAY_LENGTH($21), 2), 1))),joinType=inner)
        // DBSPMapIndexOperator 5061(892)
        let stream5061: Stream<_, IndexedWSet<Tup0, Tup1<Vec<Option<i32>>>>> = stream5057
            .map_index(
        ...
        // rel#132:LogicalJoin.(left=LogicalTableScan#67,right=LogicalTableScan#69,condition==(+(+($0, $2), $1), ITEM($21, +(/(ARRAY_LENGTH($21), 2), 1))),joinType=inner)
        // DBSPStreamJoinOperator 5065(902)
        let stream5065: Stream<
            _,
            WSet<
                Tup22<
                    Option<i32>,
        ...
        // rel#132:LogicalJoin.(left=LogicalTableScan#67,right=LogicalTableScan#69,condition==(+(+($0, $2), $1), ITEM($21, +(/(ARRAY_LENGTH($21), 2), 1))),joinType=inner)
        // DBSPFilterOperator 5070(904)
        let stream5070: Stream<
            _,
            WSet<
                Tup22<
                    Option<i32>,
        ...
        // DBSPMapOperator 5073(1010)
        let stream5073: Stream<
            _,
            WSet<
                Tup14<
                    Option<String>,
        ...
        // DBSPSinkOperator 5076(1050)
        let handle5076 = stream5073.output();

        Ok((
            handle49, handle67, handle120, handle143, handle166, handle279, handle337, handle5059,
            handle5076,
        ))
    })?;
    Ok((circuit, streams))
