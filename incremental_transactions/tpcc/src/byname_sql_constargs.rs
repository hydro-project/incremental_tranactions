...
> {
    let (circuit, streams) = Runtime::init_circuit(cconf, |circuit| {
        ...
        // DBSPFilterOperator 414
        let stream414: Stream<
            _,
            WSet<
                Tup21<
                    Option<i32>,
        ...
        // DBSPMapOperator 1348(469)
        let stream1348: Stream<
            _,
        ...
        // DBSPMapIndexOperator 1396(552)
        let stream1396: Stream<
            _,
            IndexedWSet<
                Tup2<Option<i32>, Option<i32>>,
                Tup4<Option<i32>, Option<i32>, Option<i32>, Option<String>>,
            >,
        > = stream1348.map_index(
        ...
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={0, 1},CUST_ARRAY=ARRAY_AGG($2) WITHIN GROUP ([3]))
        // DBSPStreamAggregateOperator 5210(557)
        let stream5210: Stream<
            _,
            IndexedWSet<Tup2<Option<i32>, Option<i32>>, Tup1<Vec<Option<i32>>>>,
        > = stream1396.stream_aggregate(Fold::<
            _,
            _,
        ...
        // rel#64:LogicalAggregate.(input=LogicalProject#62,group={0, 1},CUST_ARRAY=ARRAY_AGG($2) WITHIN GROUP ([3]))
        // DBSPMapOperator 5212(582)
        let stream5212: Stream<_, WSet<Tup3<Option<i32>, Option<i32>, Vec<Option<i32>>>>> = stream5210.map(move |t_6: (&Tup2<Option<i32>, Option<i32>>, &Tup1<Vec<Option<i32>>>, ), | ->
        ...
        // DBSPSinkOperator 5217(605)
        let handle5217 = stream5212.output();

        // rel#140:LogicalFilter.(input=LogicalTableScan#67,condition=AND(=($1, 43), =($2, 44)))
        // DBSPFilterOperator 666
        let stream666: Stream<
            _,
            WSet<
                Tup21<
                    Option<i32>,
        ...
        // DBSPMapIndexOperator 932
        let stream932: Stream<
            _,
            IndexedWSet<
                Tup0,
                Tup21<
        ...
        // rel#143:LogicalJoin.(left=LogicalFilter#140,right=LogicalTableScan#69,condition==($0, ITEM($23, +(/(ARRAY_LENGTH($23), 2), 1))),joinType=inner)
        // DBSPMapIndexOperator 5219(955)
        let stream5219: Stream<_, IndexedWSet<Tup0, Tup3<Option<i32>, Option<i32>, Vec<Option<i32>>>>> = stream5212.map_index(move |t_9: &Tup3<Option<i32>, Option<i32>, Vec<Option<i32>>>, | ->
        (Tup0, Tup3<Option<i32>, Option<i32>, Vec<Option<i32>>>, ) {
            (Tup0::new(), Tup3::new((*t_9).0, (*t_9).1, (*t_9).2.clone()), )
        });
        // rel#143:LogicalJoin.(left=LogicalFilter#140,right=LogicalTableScan#69,condition==($0, ITEM($23, +(/(ARRAY_LENGTH($23), 2), 1))),joinType=inner)
        // DBSPStreamJoinOperator 5223(965)
        let stream5223: Stream<
            _,
            WSet<
                Tup24<
                    Option<i32>,
        ...
        // rel#143:LogicalJoin.(left=LogicalFilter#140,right=LogicalTableScan#69,condition==($0, ITEM($23, +(/(ARRAY_LENGTH($23), 2), 1))),joinType=inner)
        // DBSPFilterOperator 5228(967)
        let stream5228: Stream<
            _,
            WSet<
                Tup24<
                    Option<i32>,
        ...
        // DBSPMapOperator 5231(1092)
        let stream5231: Stream<
            _,
            WSet<
                Tup17<
                    Option<String>,
        ...
        // DBSPSinkOperator 5234(1138)
        let handle5234 = stream5231.output();

        Ok((
            handle49, handle67, handle120, handle143, handle166, handle279, handle337, handle5217,
            handle5234,
        ))
    })?;
    Ok((circuit, streams))
